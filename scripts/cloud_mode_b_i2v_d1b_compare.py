#!/usr/bin/env python3
"""B / D1 / D1b I2V compare: hard vs latent-only vs coupled latent+timestep release.

B   = HardAnchorSchedule, timestep hard (t0=0)
D1  = LateAnchorReleaseSchedule, latent soft, timestep still hard (t0=0)
D1b = LateAnchorReleaseSchedule, latent soft + timestep soft: t0=(1-a)*t

Keeps Symmetric Circular Temporal RoPE unchanged. No D2 / no F-1 conditioning.
"""
...
OUT_ROOT = Path(
    os.environ.get(
        "MODE_B_I2V_D1B_OUT",
        "/root/latent-loop/artifacts/mode_b_i2v_d1b",
    )
)
    ...
    with (
        ...
        for step_i, t in enumerate(tqdm(timesteps)):
            ...
            if probe_dir is not None and step_i in probe_steps:
                ...
                logging.info(
                    "probe step=%s tag=%s a=%.3f t0_factor=%.3f couple=%s",
                    step_i,
                    tag,
                    a,
                    t_factor0,
                ...
                meta["frame0_timestep_factor"] = t_factor0
                meta["couple_timestep"] = couple_timestep
...
def main() -> None:
    args = parse_args()
    assert args.steps == 20, "D1/D1b table is defined for 20 steps"
    ...
    late = LateAnchorReleaseSchedule()
    variants = [
        ("B_hard_anchor", HardAnchorSchedule(), False),
        ("D1_late_release", late, False),
        ("D1b_coupled_release", late, True),
    ]
    ...
    for case_id in args.cases:
        ...
        for variant_id, anchor_sched, couple in variants:
            run_dir = out_root / case_id / variant_id
            run_dir.mkdir(parents=True, exist_ok=True)
            img.save(run_dir / "source.png")
            logging.info(
                "=== %s / %s (couple_timestep=%s) ===", case_id, variant_id, couple
            )
            t0 = time.time()
            ...
            summary = {
                ...
                "probes": probes,
                "late_diffs_l2": None if late_p is None else late_p["diffs_l2"],
            ...
            row = {
                ...
                "late_F-2->F-1": None
                if late_p is None
                else late_p["diffs_l2"]["F-2->F-1"],
                "late_F-1->0": None
                if late_p is None
                else late_p["diffs_l2"]["F-1->0"],
                "late_0->1": None if late_p is None else late_p["diffs_l2"]["0->1"],
                "late_1->2": None if late_p is None else late_p["diffs_l2"]["1->2"],
                "dir": str(run_dir),
            }
    ...
    experiment = {
        "name": "mode_b_i2v_d1b_coupled_anchor_release",
        "question": (
            "Does coupling frame-0 timestep release with latent release "
            "(D1b) flatten late F-1→0 and 0→1 vs D1 latent-only / B hard?"
        ),
