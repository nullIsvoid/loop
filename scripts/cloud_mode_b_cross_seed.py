#!/usr/bin/env python3
"""Cross-seed generalization: S1 Loopy vs S3 Symmetric only.

Freeze: no new schedules, no eval-system expansion.
Scenes: pendulum, rotating_fan, human_sway
Seeds: 123, 888
Shared: 1280x704, 81 frames (F=21), 20 steps, CFG=5, UniPC
"""
...
OUT_DIR = Path(
    os.environ.get(
        "MODE_B_CROSS_SEED_OUT",
        "/root/latent-loop/artifacts/mode_b_cross_seed",
    )
)
...
def main() -> None:
    ...
    for seed in SEEDS:
        for scene_id, prompt in SCENES:
            for sched_key, sched in _schedules():
                run_id = f"seed{seed}__{scene_id}__{sched_key}"
    ...
    experiment = {
        "name": "mode_b_cross_seed_s1_vs_s3",
        "research_question": (
            "Does SymmetricShiftSchedule (S3) generalize across seeds vs "
            "LoopyShiftSchedule (S1) on phase-clear motion?"
        ),
        "freeze": {
            "no_new_schedules": True,
            "no_eval_system_expansion": True,
            "metrics_auxiliary_only": True,
            "s2_fixed1_parked": True,
        },
        "shared": {
            "seeds": SEEDS,
            "size_wh": list(SIZE),
        ...
        "schedules": ["S1_loopy", "S3_symmetric"],
        "schedule_tables": schedule_tables,
        "runs": runs,
        "git_policy": "Commit x3 previews + metadata only; full mp4s stay on cloud.",
        "human_check": (
            "For each of 6 pairs (seed×scene), compare S1 vs S3 x3 on seam only. "
            "If S3 wins or ties most pairs → promote Symmetric as Wan default."
        ),
    }
    ...
    print(
        json.dumps(
            {"ok": True, "n_runs": len(runs), "out_dir": str(out_dir)}, indent=2
        )
    )
    print("CROSS_SEED_OK")
